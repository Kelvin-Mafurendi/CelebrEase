...
class EditProfile extends StatefulWidget {
  ...
  const EditProfile({
    ...
    required this.userType, required Map initialData,
  });
...
class _EditProfileState extends State<EditProfile> {
  ...
  Future<void> initializeControllers() async {
    ...
    if (user != null) {
      final docSnapshot = await _firestore.collection(widget.userType).doc(user.uid).get();
      if (docSnapshot.exists) {
        ...
        setState(() {
          ...
          phoneNumberController = TextEditingController(text: data['phone number'] ?? '');
          brandController = TextEditingController(text: data['business name'] ?? '');
          aboutController = TextEditingController(text: data['business description'] ?? '');
            ...
            child: Column(
              children: [
                ...
                if (widget.userType == 'Vendors') ...[
                  ...
                  buildTextField(aboutController, 'About Business', maxLines: 5),
                  buildTimeField('Start Time', selectedStartTime, (time) => setState(() => selectedStartTime = time)),
                  buildTimeField('End Time', selectedCloseTime, (time) => setState(() => selectedCloseTime = time)),
                ],
  ...
  Widget buildTimeField(String label, String? selectedTime, Function(String?) onChanged) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0),
      child: TextFormField(
        decoration: InputDecoration(
  ...
  void updateProfile(BuildContext context) {
    if (_formKey.currentState!.validate()) {
      ...
      updateIfChanged('surname', surnameController.text, profileData['surname']);
      updateIfChanged('email', emailController.text, profileData['email']);
      updateIfChanged('phone number', phoneNumberController.text, profileData['phone number']);
      ...
      if (widget.userType == 'Vendors') {
        updateIfChanged('business name', brandController.text, profileData['business name']);
        updateIfChanged('business description', aboutController.text, profileData['business description']);
        updateIfChanged('category', categoryController.text, profileData['category']);
        updateIfChanged('address', addressController.text, profileData['address']);
        updateIfChanged('startTime', selectedStartTime!, profileData['startTime']);
        updateIfChanged('endTime', selectedCloseTime!, profileData['endTime']);
      }

      if (updatedData.isNotEmpty) {
        Provider.of<ChangeManager>(context, listen: false).loadProfiledata(updatedData, widget.userType);
